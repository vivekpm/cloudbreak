...
public class ResourcePollingService extends AbstractPollingService {
    private static final Logger LOGGER = LoggerFactory.getLogger(ResourcePollingService.class);
    ...
    protected PollingInfo fetchCloudPollingInfo(PollingInfo pollingInfo) {
        ...
        List<CloudResourceStatus> results = resourcePollingInfo.getConnector().resources()
                .check(resourcePollingInfo.authenticatedContext(), ResourceLists.transform(resourcePollingInfo.getCloudResourceStatus()));
    ...
    public PollingInfo handleFailure(PollingInfo pollingInfo) {
        LOGGER.debug("Handling failure for pollinginfo: {}", pollingInfo);
        return pollingInfo;
    }
